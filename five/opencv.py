...
def imageProcessing(path):
    try:
        ...
        """
        # 框选图片
        image, contours, hierarchy = cv2.findContours(dilated, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        ...
        # 进行筛选
        IDcnts = widthSelect(contours)

        """
            函数cv2.boundingRect() 选择一个与边框平行的矩形包围图像
                不考虑对象的旋转，所以边界的矩形区域不会是最小的
        """
        # 裁剪图像
        for index, IDcnt in enumerate(IDcnts):
            x, y, width, height = cv2.boundingRect(IDcnt)
            # IDimg = img[y: y + height, x: x + width]
            IDimg = img[y - 30: y + height + 30, x - 30: x + width + 30]

        if not os.path.exists('temp'):
            os.makedirs('temp')
        tempPath = "temp/prosessing.jpg"
        ...
        # 获取图片长宽 缩放 调试用
        height, width = IDimg.shape[:2]

        if (height < 100 | width < 1000):
            print "图片处理失败"

        dstWidth = width
        dstHeight = height

        print "IDimgDstHeight:", dstHeight, "IDimgDstHeight:", dstWidth

        IDimg = cv2.resize(IDimg, (dstWidth, dstHeight))
        gray = cv2.resize(gray, (dstWidth, dstHeight))
        ...
        # 测试用 画框
        # cv2.drawContours(IDimg, contours, -1, (255, 0, 0), 3)

        # 测试用 显示图片
        # cv2.namedWindow("IDimg", cv2.WINDOW_NORMAL)
        ...
        for index, IDcnt in enumerate(IDcnts):
            ...
            box1 = np.int0(box)
            cv2.drawContours(IDimg, [box1], 0, (100, 100, 100), 2)
        ...
        # 蓝(0, 0, 255)
        # 白(255, 255, 255)
        # cv2.circle(IDimg, (first_x, first_y), 2, (255, 0, 0), 3)
        # cv2.circle(IDimg, (second_x, second_y), 2, (0, 255, 0), 3)
        # cv2.circle(IDimg, (third_x, third_y), 2, (0, 0, 255), 3)
        # cv2.circle(IDimg, (fourth_x, fourth_y), 2, (255, 255, 255), 3)

        # 显示图片 测试用
        # cv2.namedWindow("IDimg2", cv2.WINDOW_NORMAL)
        # cv2.imshow("IDimg2", IDimg)
        # cv2.waitKey(0)
        ...
        dstProjection = cv2.warpPerspective(IDimg, M, (dstImgWidth, dstImgHeigt))
